...
bool Af::getAverageAndTestIr(const RgbyRegions &awbStats, double rgb[3])
{
	...
	for (unsigned i = 0; i < awbStats.numRegions(); ++i) {
		...
		if (w) {
			sw += w;
			sr += w * awbStats.get(i).val.rSum;
			sg += w * awbStats.get(i).val.gSum;
			sb += w * awbStats.get(i).val.bSum;
		}
		if (cfg_.checkForIR) {
			if (4 * r < 5 * b && 4 * b < 5 * r &&
			    4 * r < 5 * g && 4 * g < 5 * r &&
...
void Af::doAF(double contrast, double phase, double conf)
{
	...
	if (scanState_ < ScanState::Coarse1 && mode_ == AfModeContinuous) {
		...
		if (stepCount_ > 0)
			...
			dropCount_ = 0;
			sceneChangeCount_ = 0;
			oldSceneContrast_ = scanMaxContrast_;
			scanData_.clear();
		} else if (conf >= 1.5 * cfg_.confThresh && earlyTerminationByPhase(phase)) {
			std::copy(prevAverage_, prevAverage_ + 3, oldSceneAverage_);
			scanState_ = ScanState::Settle;
